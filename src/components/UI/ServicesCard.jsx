import PropTypes from 'prop-types';
import { Link } from "react-router-dom";
import { useLanguage } from "@store/LanguageContext";
import '@styles/services.css';
import { ArrowLeft, ArrowRight } from 'lucide-react';

/**
 * Renders a service card component.
 *
 * @param {object} props - The component props.
 * @param {object} props.service - The service object.
 * @param {string} props.service.slug - The URL of the service.
 * @param {string} props.service.name - The title of the service.
 * @param {string} props.service.description - The description of the service.
 * @param {string} props.service.img - The image URL of the service.
 * @param {string} [props.className] - Additional CSS classes for the card.
 * @param {string} [props.titleColor] - Additional CSS classes for the title.
 * @param {string} [props.textColor] - Additional CSS classes for the text.
 * @param {string} [props.linkColor] - Additional CSS classes for the link.
 * @returns {JSX.Element} The rendered service card component.
 */
const ServicesCard = ({
    service,
    className = '',
    titleColor,
    textColor,
    linkColor
}) => {
    const { t, direction } = useLanguage();

    return (
        <div className={`card service-box !w92.5 h-125 md:h-148 lg:h-150.5 px-3 bg-[#020101]/24 ${className}`}>
            <div className="card-body pb-1 px-2.5 items-center text-center">
                <Link
                    to={`/services/${service.slug}`}
                    aria-label={service.name}
                    className={`card-title pb-6 ${direction === 'rtl' ? 'text-xl' : '!text-2xl'} !font-bold !font-spartan ${titleColor} leading-7.5`}
                >
                    {service.name}
                </Link>
                <p dir={direction} className={`${textColor}`}>{service.description}</p>
            </div>
            <div className="icon-serv">
                <img src="/assets/images/icons/thumbs-up.png" alt="service icon" loading="lazy" className="icon-thumbs" />
            </div>
            <div className="card-overlay-img">
                <figure>
                    <img
                        src={service.img}
                        alt={service.name}
                        className="relative rounded-[0.625rem] w-full !h-70 md:!h-83.5 object-cover"
                        loading="lazy"
                        width="500"
                        height="500"
                        srcSet={`${service.img} 500w, ${service.img} 300w, ${service.img} 100w, ${service.img} 150w`}
                        sizes="(max-width: 500px) 100vw, 500px"
                    />
                </figure>
            </div>
            <Link
                to={`/services/${service.slug}`}
                aria-label={service.name}
                className={`pt-7 flex items-center justify-center gap-1 ${direction === 'rtl' ? 'flex-row-reverse font-medium' : 'flex-row-reverse'} capitalize ${linkColor} text-center`}
            >
                {t('services.readMore')} <span className="sr-only">about {service.name}</span>
                {direction === 'rtl' ? (
                    <ArrowLeft size={18} strokeWidth={2} />
                ) : (
                    <ArrowRight size={18} strokeWidth={2} />
                )}
            </Link>
        </div>
    );
}

ServicesCard.propTypes = {
    service: PropTypes.object.isRequired,
    className: PropTypes.string,
    titleColor: PropTypes.string,
    textColor: PropTypes.string,
    linkColor: PropTypes.string,
};

export default ServicesCard;